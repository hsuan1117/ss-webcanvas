    ...
    return (
        <Dialog open={isOpen} onClose={() => setIsOpen(false)} as={"div"}
            ...
            <Dialog.Panel className={"shadow-lg px-4 py-2 bg-sky-300 w-3/5"}>
                <Dialog.Title className={"text-center text-2xl font-bold"}><FontAwesomeIcon icon={faInfoCircle} /> 關於 Web小畫家</Dialog.Title>
                <Dialog.Description className={"mt-6"}>
                    Made with ❤️ by <a href={"https://github.com/hsuan1117/"} target={"_blank"} className={"underline"}>hsuan1117</a>
                </Dialog.Description>
                ...
                <button onClick={() => setIsOpen(false)} className={"inline-flex flex-row justify-center items-center w-full bg-green-500 py-2 mt-6 mb-2 shadow-lg"}>
...
export default function MenuBar({saveFile, aboutUs}) {
    ...
    return (
        <>
            <Menu className={"relative inline-block z-20"} as={"div"}>
                <div>
                    <Menu.Button
                        className="inline-flex justify-center rounded-md mx-2 px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-200">
                        檔案
                    </Menu.Button>
                </div>
                <Menu.Items className={"absolute left-2 py-3 w-48 z-10 bg-gray-200 shadow-xl rounded-md flex flex-col"}>
                    <Menu.Item>
                        {({active}) => (
                            <button
                                className={`block inline-flex items-center gap-1 px-4 py-3 ${active && 'bg-gray-300'}`}
                            >
